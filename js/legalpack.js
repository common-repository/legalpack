jQuery(document).ready(function ($) {
    ...
    var topContainer = jQuery("#legalpack-top-fixed-container");
    var bottomContainer = jQuery("#legalpack-bottom-fixed-container");
    var z = maxZIndex();
    topContainer.css('z-index', z + 1);
    bottomContainer.css('z-index', z + 2);
    jQuery("#legalpack-top-static-container").css("margin-top",
        parseInt(getShadowProperty("body", ":before", "height", 0)) + "px");
    jQuery("#legalpack-bottom-static-container").css("margin-bottom",
        parseInt(getShadowProperty("body", ":after", "height", 0)) + "px");
    function recalcContainers() {
        $("#legalpack-top-fixed-style,#legalpack-bottom-fixed-style").remove();
        var h = $("head");
        var topContainer = jQuery("#legalpack-top-fixed-container");
        var bottomContainer = jQuery("#legalpack-bottom-fixed-container");
        if (topContainer.length) {
            h.append('<style id="legalpack-top-fixed-style">body:before{top:' +
                parseInt(topContainer.height()) + 'px !important;}</style>');
        }
        if (bottomContainer.length) {
            h.append('<style id="legalpack-bottom-fixed-style">body:after{bottom:' +
    ...
    $(".legalpack-notice-close").click(function () {
        var t = jQuery(this);
