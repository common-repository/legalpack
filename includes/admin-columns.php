...
namespace legalpack {
	...
	abstract class Admin_Columns {
		...
		static function init () {
			add_filter ('manage_edit-' . LEGALPACK_CPT . '_columns', array (__CLASS__, 'edit_columns'));
			add_filter ('manage_edit-' . LEGALPACK_CPT . '_sortable_columns', array (
				__CLASS__,
				'sortable_columns'
			));
			add_filter ('display_post_states', array (__CLASS__, 'display_post_states'), 10, 2);
			add_action ('manage_' . LEGALPACK_CPT . '_posts_custom_column',
		...
		static function edit_columns ($columns) {
			return array (
				...
				static::COL_TITLE => __ ('Title', LEGALPACK_SLUG),
				static::COL_STATUS => __ ('Status', LEGALPACK_SLUG),
				static::COL_LAST_DATE => __ ('Last Effective Date', LEGALPACK_SLUG),
				static::COL_DATE => __ ('Date', LEGALPACK_SLUG)
			);
		}
			...
			switch ($column) {
				case static::COL_LAST_DATE:
					if ($post->post_status == 'publish') {
						echo get_post_modified_time (get_option ('date_format'), false, $post, true);
					}
					break;
			}
		}
